...
class TestMain:
  ...
  def test_main_has_tick_counter(self):
    assert_equal(self.a_main.config.flow_counter.flow_ticks, 0)

  def test_main_can_update_readings_set_with_flow_ticks(self):
    assert_equal(self.a_main.flow_reader.readings_set, [0])
    self.a_main.flow_reader.take_reading(self.a_main.config.flow_counter.flow_ticks)
  ...
  def test_main_updates_reading_set_in_timed_thread(self):
    ...
    assert_equal(self.a_main.flow_reader.readings_set, [0])
    self.a_main.config.flow_counter.flow_ticks = 8
    self.a_main.start_readings_thread()
    time.sleep(0.6)
    assert_equal(self.a_main.flow_reader.readings_set, [0,8])

  def test_main_averages(self):
    assert_equal(self.a_main.config.readings_calculator.calculate_average([8,4,6]), 6)
